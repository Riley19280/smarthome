...
//**************************************************************************************************************
void asyncHTTPrequest::onDisconnect(onDisconnectCB cb, void* arg){
    DEBUG_HTTP("onDisconnect() CB set\r\n");
    _onDisconnectCB = cb;
    _onDisconnectCBarg = arg;
}
...
void  asyncHTTPrequest::_setReadyState(readyStates newState){
    if(_readyState != newState) {
        int oldState = _readyState;
        _readyState = newState;          
        DEBUG_HTTP("_setReadyState(%d)\r\n", _readyState);
        if(_readyStateChangeCB){
            _readyStateChangeCB(_readyStateChangeCBarg, this, newState, oldState);
        }
    } 
}
...
void  asyncHTTPrequest::_onDisconnect(AsyncClient* client){
    ...
    if(_onDisconnectCB){
        _onDisconnectCB(_onDisconnectCBarg, this);
    }
    _release;
}
